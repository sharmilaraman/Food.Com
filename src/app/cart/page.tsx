  ...
  return (
    <div className="h-[calc(100vh-6rem)] md:h-[calc(100vh-9rem)] flex flex-col text-red-500 lg:flex-row">
      ...
      <div className="h-1/2 p-4  flex flex-col justify-center overflow-scroll lg:h-full lg:w-2/3 2xl:w-1/2 lg:px-20">
        {items.map((item) => (
          <div className="flex gap-2 items-center md:justify-between mb-4" key={`${item.id}-${item.size}`}>
            <Image src={item.img || "/products/p1.png"} alt="" width={100} height={100} />
            <div className="">
              <h1 className="uppercase text-md md:text-xl font-bold">{item.title}</h1>
              <span>{item.size}</span>
            </div>
